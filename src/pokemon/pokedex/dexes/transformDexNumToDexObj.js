...
function transformDexNumToDexObj() {
  let start = 1;
  // for each dex in numbers add the dex in objects to dexes_objects folder
  ...
  let saveRoute = "./dexes_objects/";
  if (isSingleDigit(start)) {
    saveRoute += `0${start}_${key}.json`;
  } else {
    saveRoute += `${start}_${key}.json`;
  }
  start++;
